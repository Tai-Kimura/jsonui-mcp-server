...
export function register(server: McpServer, config: ServerConfig) {
  server.tool(
    ...
    "Generate a screen test file template via the jsonui-test CLI",
    {
      name: z.string().describe("Screen name (e.g., login, home)"),
      path: z.string().optional().describe("Output test file path (default: tests/screens/<name>/<name>.test.json)"),
      platform: z
    ...
    async (params) => {
      try {
        ...
        const args = ["generate", "test", "screen", params.name];
